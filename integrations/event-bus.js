...
const EVENT_BUS_CONFIG = {
    ...
    eventTypes: {
        ...
        CREDIT: {
            ...
            TRANSFERRED: 'credit:transferred'
        },
        REPORT: {
        ...
        LAUDO: {
            GENERATED: 'laudo:generated',
            SIGNED: 'laudo:signed',
            DELIVERED: 'laudo:delivered',
...
class EventBus {
    ...
    _saveEvents() {
        try {
            ...
            console.warn('⚠️ Erro ao salvar eventos:', error);
        }
    }
    ...
    emit(event, data) {
        try {
            ...
            if (this.listeners.has(event)) {
                ...
                // Separar callbacks síncronos e assíncronos
                for (var i = 0; i < callbacks.length; i++) {
                    var cb = callbacks[i];
                    if (cb.async) {
                        asyncCallbacks.push(cb);
                    } else {
                        syncCallbacks.push(cb);
                    }
                }
                
                // Executar callbacks síncronos
                for (var j = 0; j < syncCallbacks.length; j++) {
                    try {
                        syncCallbacks[j](data);
                    } catch (error) {
                        console.error('❌ Erro no callback síncrono:', error);
                    }
                }
                
                // Executar callbacks assíncronos
                for (var k = 0; k < asyncCallbacks.length; k++) {
                    setTimeout(function(callback) {
                        try {
                            callback(data);
                        } catch (error) {
                            console.error('❌ Erro no callback assíncrono:', error);
                        }
                    }, 0, asyncCallbacks[k]);
                }
            }
            ...
            if (window.moduleConnector) {
                window.moduleConnector._emit(event, data);
            }
    ...
    removeAllListeners(event) {
        try {
            if (event) {
                this.listeners.delete(event);
                console.log(`👂 Todos os listeners removidos para: ${event}`);
            } else {
    ...
    getEvents(filter) {
        try {
            ...
            if (filter) {
                if (filter.event) {
                    events = events.filter(function(e) { return e.event === filter.event; });
                }
                if (filter.startDate) {
                    events = events.filter(function(e) {
                        return new Date(e.timestamp) >= new Date(filter.startDate);
                    });
                }
                if (filter.endDate) {
                    events = events.filter(function(e) {
                        return new Date(e.timestamp) <= new Date(filter.endDate);
                    });
                }
            }
    ...
    getStats() {
        ...
        for (var [key, value] of this.listeners) {
            stats.listeners[key] = value.length;
        }
