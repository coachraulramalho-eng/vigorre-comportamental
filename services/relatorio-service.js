...
class RelatorioService {
    ...
    gerarSimplificado(dados) {
        try {
            ...
            return {
                success: true,
                data: relatorio,
                message: 'Relatório simplificado gerado com sucesso'
            };
    ...
    gerarExecutivo(dados) {
        try {
            if (!dados || !dados.empresaId) {
                return { success: false, error: 'Dados da empresa são obrigatórios' };
            }
        ...
        try {
            ...
            for (var i = 0; i < relatorios.length; i++) {
                if (relatorios[i].id === id) {
                    return { success: true, data: relatorios[i] };
                }
            }
    ...
    listar(filtros) {
        try {
            ...
            if (filtros) {
                if (filtros.tipo) {
                    relatorios = relatorios.filter(function(r) { return r.tipo === filtros.tipo; });
                }
                if (filtros.participantId) {
                    relatorios = relatorios.filter(function(r) { return r.participantId === filtros.participantId; });
                }
                if (filtros.empresaId) {
                    relatorios = relatorios.filter(function(r) { return r.empresaId === filtros.empresaId; });
                }
                if (filtros.companyId) {
                    relatorios = relatorios.filter(function(r) { return r.companyId === filtros.companyId; });
                }
            }
    ...
    exportarPDF(relatorioId) {
        try {
            ...
            // Em produção, usar html2pdf.js ou similar
    ...
    excluir(id) {
        try {
            ...
            if (filtered.length === relatorios.length) {
                return { success: false, error: 'Relatório não encontrado' };
            }
    ...
    _gerarId() {
        return 'R' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6).toUpperCase();
    }
    ...
    _gerarResumo(dados) {
        var resumo = {
            disc: 'Perfil DISC identificado',
            ie: 'Inteligência Emocional em desenvolvimento',
        ...
        if (dados.ie && dados.ie.overallPercentage) {
            var nivel = dados.ie.overallPercentage >= 80 ? 'Excelente' :
                        dados.ie.overallPercentage >= 60 ? 'Bom' :
    ...
    _gerarAnalise(dados) {
        return {
            pontosFortes: ['Comunicação eficaz', 'Pensamento analítico', 'Resiliência'],
    ...
    _gerarHTML(relatorio) {
        return `
            ...
            <head>
                <meta charset="UTF-8">
                <title>Relatório - VIGORRE ONE™</title>
                <style>
                    ...
                    .logo { font-size: 24px; font-weight: 700; color: #0A2540; }
                    .logo span { color: #D97706; }
                    .subtitle { color: #64748B; font-size: 14px; }
                    .content { margin-top: 30px; }
                    .section { margin-bottom: 30px; }
                    .section-title { font-size: 18px; font-weight: 600; color: #1D4ED8; border-bottom: 1px solid #E2E8F0; padding-bottom: 8px; }
            ...
            <body>
                <div class="header">
                    ...
                    <div class="subtitle">Relatório ${relatorio.tipo.toUpperCase()}</div>
                ...
                <div class="content">
                    <div class="section">
                        ...
                        <p><strong>Nome:</strong> ${relatorio.participantName || '--'}</p>
                        <p><strong>Empresa:</strong> ${relatorio.companyName || '--'}</p>
                    </div>
                    <div class="section">
                        <div class="section-title">📈 Resumo</div>
                        ${Object.keys(relatorio.dados || {}).map(function(key) {
                            return '<div class="card"><strong>' + key.toUpperCase() + '</strong>: ' + JSON.stringify(relatorio.dados[key]) + '</div>';
                        }).join('')}
                    </div>
                </div>
                <div class="footer">
    ...
    _gerarDadosExcel(relatorio) {
        return {
            headers: ['ID', 'Participante', 'Empresa', 'Tipo', 'Data'],
            rows: [[
                relatorio.id,
                relatorio.participantName || '--',
                relatorio.companyName || '--',
                relatorio.tipo,
